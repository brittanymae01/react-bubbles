...
A web authentication process that lets users enter their username and password once and receive a uniquely-generated encrypted token in exchange. This token is then used to access protected pages or resources instead of the login credentials for a designated period of time.

- [x] What steps can you take in your web apps to keep your data secure?

We can use protected routes and send an authentication header with our API calls.

- [x] Describe how web servers work.

The primary function of a web server is to store, process and deliver web pages to people when a request is made from the client side. This is usually done by the browser which requests the file via HTTP. 
